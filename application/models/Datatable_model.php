 ...
 */
public function get_rows ( $request, $table, $primaryKey, $columns, $join_where=NULL )
{
	$bindings = array();
	// $arr = $this->db->item('default');
	...
	if( $where == '' && $join_where == NULL){
		...
		$data = $this->sql_exec( $db, $bindings,
			"SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", $this->pluck($columns, 'db'))."`
			 FROM `$table` " . $join_where . " " . $order . " " . $limit
		);
	}else if( $where != '' && $join_where != NULL ){
		$data = $this->sql_exec( $db, $bindings,
			"SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", $this->pluck($columns, 'db'))."`
			 FROM `$table` " . " " . $join_where . " "  . $where . " " . $order . " " . $limit
		);
	}
